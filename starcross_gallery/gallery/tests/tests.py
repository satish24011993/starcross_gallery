...
class ImageTests(TestCase):
    ...
    test_image_title = "Antibies Marina"
    test_slug = "antibes-marina"
    test_album_title = "My First Album"
    test_image_title_unicode = "Паровоз"

    image_filenames = ['antibes_marin.jpg', 'castle_combie.jpg', 'паровоз.jpg']
    ...
    def setUp(self):
        ...
        User.objects.create_superuser(
            self.username, 'user@gmail.com', self.password)
    ...
    def test_image_list(self):
        ...
        self.assertContains(response, self.test_image_title,
                            msg_prefix="Image title missing from image feed")
    ...
    def test_image_detail(self):
        ...
        response = self.client.get(reverse('gallery: image_detail', kwargs={
                                   'pk': self.image.pk, 'slug': self.image.title}))
        self.assertContains(response, self.test_image_title,
                            msg_prefix="Image preview does not contain image title")
    ...
    def test_album_image_detail(self):
        response = self.client.get(reverse('gallery:album_image_detail', kwargs={
                                   'pk': self.image.pk, 'slug': self.image.title, 'apk': self.album.pk}))
        self.assertContains(response, self.test_album_title,
                            count=2, msg_prefix="Image preview incorrect")
    ...
    def test_empty_album(self):
        ...
        self.assertEqual(response.status_code, 200,
                         "Error displaying empty album")
        Album.objects.all().last().delete()

    # Test albums contain images
    def test_album_view(self):

        response = self.client.get(reverse('gallery:album_detail', kwargs={
                                   'pk': self.album.pk, 'slug': self.album.title}))
        self.assertEqual(response.status_code, 200, "Error testing album view")
        self.assertContains(response, self.image.title, count=2,
                            msg_prefix="Error testing image in album view")

    def test_image_properties(self):

        image = Image.objects.all()[0]
        self.assertEqual(image.title, ImageTests.test_image_title,
                         "Incorrect title in image object")
        self.assertEqual(image.date_taken, datetime.strptime(
            "2013-03-21 15:04:53", "%Y-%m-%d %H:%M:%S"), "Incorrect date in image object")
        self.assertEqual(image.slug, slef.test_slug,
                         "Incorrect slug in image object")
    ...
    def test_image_upload(self):
        ...
        self.assertEqual(response.status_code, 200,
                         "Error testing empty image upload")

        # Invalid file
        data = {'data': SimpleUploadedFile('text.txt', b'text')}
        response = self.client.post(reverse('gallery:image_upload'), data=data)
        self.assertContains(response, "Unable to add invalid images",
                            msg_prefix="Error testing invalid image data")

        # Valid data
        with open(image_path, 'rb') as image_file:
            data = {'apk': self.album.pk, 'data': image_file}
            response = self.client.post(
                reverse('gallery:image_upload'), data=data)
        self.assertRedirects(response, reverse(
            'gallery:image_list'), msg_prefix="Error uploading image to album")
        self.assertEqual(album_size + 1, len(self.album.images.all()),
                         "Error uploading image to album")
        self.album.images.last().delete()
        self.assertEqual(album_size, len(
            self.album.images.all()), "Error removing image")
