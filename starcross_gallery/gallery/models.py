...
class Image(models.Model):
    ...
    data_preview = ImageSpecField(source='data', processors=[ResizeToFit(
            ...
            if hasattr(img, '_getexif'):
                ...
                if 'ExposureTime' in exif_data:
                    exif_data['Exposure'] = "{0}/{1}".format(
                        exif_data['ExposureTime'].numerator, exif_data['ExposureTime'].denominator)
    ...
    def date_taken(self):
        ...
        except ValueError:  # Fll back to file modification time
            return self.mtime
...
class Album(models.Model):
    ...
    images = models.ManyToManyField(
        Image, blank=True, related_name='image_albums')
    highlight = models.OneToOneField(
        Image, related_name='album_highlight', null=True, blank=True, on_delete=models.SET_NULL,)
    ...
    def display_highlight(self):
        # if there is no highlight but there are imagges in the album, use the first
